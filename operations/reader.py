...
from entities.ProjReportFull import ProjReportFull
import datetime
import errno
...
class Reader():
    ...
    _SUB_CONTRACT_NAME = r"工程分包合同查询20250821111157.xlsx"
    _MAIN_CONTRACT_NAME = r"工程总包合同查询20250821110956.xlsx"
    _PROJ_TABLE_NAME = r"工程详情信息导出数据-2025-08-20.xlsx"
    ...
    def readSubContract(self):
        ...
        subContract = SubContract(os.path.join(self._CURR_PROJ_PATH, self._DATA_PATH), self._SUB_CONTRACT_NAME,
                                  self._SUB_CONTRACT_SHEET_NAME)
    ...
    def readMainContract(self):
        ...
        subContract = MainContract(os.path.join(self._CURR_PROJ_PATH, self._DATA_PATH), self._MAIN_CONTRACT_NAME,
                                   self._MAIN_CONTRACT_NAME)
    ...
    def readProjectReport(self):
        ...
        proj_table = ProjectReport(os.path.join(self._CURR_PROJ_PATH, self._DATA_PATH), self._PROJ_TABLE_NAME,
                                   self._PROJ_REPORT_SHEET_NAME,None)
    ...
    def readFullProjReport(self):
        print(
            f"运行读取合同全量表，文件名：\n- {self._PROJ_TABLE_NAME}\n报告生成日期：{self._DATETIME_TODAY}...")
        fullProjReport = ProjReportFull(os.path.join(self._CURR_PROJ_PATH, self._DATA_PATH), self._PROJ_TABLE_NAME,
                                        self._PROJ_REPORT_SHEET_NAME, None)
        df_fullProjReport = fullProjReport.importExcelSheet()
        return df_fullProjReport
